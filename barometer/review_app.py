...
<script>
const CSRF={token_json};let data={{items:[],meta:null}},filter='pending',sortMode='priority',selected=null;
...
function renderFilters(){{const names=['pending','reclassify','all','approved','corrected','rejected','deferred'];document.querySelector('#filters').innerHTML=names.map(n=>`<button class="${{filter===n?'active':''}}" data-filter="${{n}}">${{n}}</button>`).join('')+`<select id="queue-sort" aria-label="Queue order"><option value="priority" ${{sortMode==='priority'?'selected':''}}>Temporal priority</option><option value="newest" ${{sortMode==='newest'?'selected':''}}>Newest first</option></select>`;document.querySelectorAll('[data-filter]').forEach(b=>b.onclick=()=>{{filter=b.dataset.filter;selected=null;render()}});document.querySelector('#queue-sort').onchange=e=>{{sortMode=e.target.value;selected=null;render()}}}}
function visible(){{const items=data.items.filter(i=>filter==='all'||decisionStatus(i)===filter);return items.sort((a,b)=>sortMode==='priority'?(b.temporal_priority.score-a.temporal_priority.score)||b.received_at.localeCompare(a.received_at):b.received_at.localeCompare(a.received_at))}}
