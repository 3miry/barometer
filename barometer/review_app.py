...
from .reviews import ReviewStore, source_fingerprint
    ...
    for row in load_source_reports(source_db):
        ...
        items.append({
            ...
            "proposal": {
                ...
                "novelty_candidates": list(result.novelty_candidates),
...
.qitem{{display:block;width:100%;text-align:left;background:transparent;color:var(--ink);border:0;border-bottom:1px solid rgba(255,255,255,.06);padding:12px;border-radius:9px}}.qitem:hover,.qitem.active{{background:#172832}}.qtop{{display:flex;justify-content:space-between;gap:8px}}.qid{{font-family:ui-monospace,monospace;font-size:11px;color:var(--blue)}}.qtext{{color:#c3cdd2;margin-top:6px;font-size:12px;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}}
...
.report{{white-space:pre-wrap;background:#0b151b;border:1px solid #243843;border-radius:13px;padding:16px;color:#d7e0e4;max-height:270px;overflow:auto}}.section{{margin-top:20px;padding-top:18px;border-top:1px solid rgba(255,255,255,.08)}}.section h2{{font-size:15px;margin:0 0 10px}}
...
function metrics(){{const counts={{pending:0,approved:0,corrected:0,rejected:0,deferred:0}};data.items.forEach(i=>counts[decisionStatus(i)]++);document.querySelector('#metrics').innerHTML=[['Pending',counts.pending],['Reviewed',data.items.length-counts.pending],['Attribution review',data.items.filter(i=>i.attribution_status.includes('review')||i.attribution_status.includes('mismatch')).length],['Novelty',data.items.filter(i=>i.proposal.novelty_candidates.length).length]].map(([k,v])=>`<div class="metric"><b>${{v}}</b><span>${{k}}</span></div>`).join('')}}
...
function renderQueue(){{const items=visible();if(!items.some(i=>i.report_id===selected))selected=items[0]?.report_id||null;document.querySelector('#queue').innerHTML=items.length?items.map(i=>`<button class="qitem ${{selected===i.report_id?'active':''}}" data-id="${{esc(i.report_id)}}"><div class="qtop"><span class="qid">${{esc(i.report_id)}}</span><span class="pill ${{decisionStatus(i)==='pending'?'warn':'good'}}">${{esc(decisionStatus(i))}}</span></div><div class="qtext">${{esc(i.text)}}</div></button>`).join(''):'<div class="empty" style="min-height:180px">Nothing in this view.</div>';document.querySelectorAll('.qitem').forEach(b=>b.onclick=()=>{{selected=b.dataset.id;render()}})}}
...
function renderDetail(){{const i=data.items.find(x=>x.report_id===selected),d=document.querySelector('#detail');if(!i){{d.className='detail empty';d.textContent='Choose a report.';return}}d.className='detail';const saved=i.decision&&!i.decision.stale?i.decision:null;i._edit=i._edit||{{family:saved?.target_family??i.stored_family,variant:saved?.target_variant??i.stored_variant,observations:clone(saved?.observations??i.proposal.observations),novelty:clone(saved?.novelty_candidates??i.proposal.novelty_candidates),note:saved?.review_note||''}};const e=i._edit;const attrBad=i.attribution_status!=='single_family'&&i.attribution_status!=='single_variant';d.innerHTML=`<div class="eyebrow">${{esc(i.proposal.classifier_version)}} · ${{esc(i.received_at)}}</div><h1>${{esc(i.source)}} report</h1><div class="badges"><span class="pill">stored: ${{esc(i.stored_variant||i.stored_family)}}</span><span class="pill ${{attrBad?'warn':'good'}}">${{esc(i.attribution_status)}}</span><span class="pill">${{esc(i.proposal.eligibility)}}</span>${{i.proposal.abstention_reason?`<span class="pill warn">${{esc(i.proposal.abstention_reason)}}</span>`:''}}</div><div class="report">${{esc(i.text)}}</div><div class="section"><h2>Attribution</h2><div class="grid"><div><label>Target family</label><select id="family">${{Object.entries(data.meta.families).map(([k,v])=>`<option value="${{k}}" ${{k===e.family?'selected':''}}>${{esc(v.lab)}} · ${{esc(v.label)}}</option>`).join('')}}</select></div><div><label>Exact model</label><select id="variant">${{variantOptions(e.family,e.variant)}}</select></div></div></div><div class="section"><h2>Governed observations</h2><div id="observations">${{e.observations.map(obsEditor).join('')||'<div class="sub">No governed observations selected.</div>'}}</div><div class="addrow"><select id="add-concept"><option value="">Add a governed concept…</option>${{data.meta.concepts.filter(c=>!e.observations.some(o=>o.concept_id===c.id)).map(c=>`<option value="${{c.id}}">${{esc(c.label)}} · ${{esc(c.parent||c.reporting_layer)}}</option>`).join('')}}</select><button id="add">Add</button></div></div><div class="section"><h2>Unresolved novelty</h2><label>Comma-separated private candidate labels</label><input id="novelty" value="${{esc(e.novelty.join(', '))}}"><label style="margin-top:10px">Reviewer note</label><textarea id="note">${{esc(e.note)}}</textarea></div><div class="actions"><button class="approve" data-save="approved">Approve proposal</button><button class="correct" data-save="corrected">Save correction</button><button data-save="deferred">Defer</button><button class="reject" data-save="rejected">Reject</button><span class="status" id="save-status"></span></div>`;
...
async function save(i,status){{const e=i._edit,s=document.querySelector('#save-status');e.novelty=document.querySelector('#novelty').value.split(',').map(v=>v.trim()).filter(Boolean);e.note=document.querySelector('#note').value;s.className='status';s.textContent='Saving…';const payload={{status,target_family:e.family,target_variant:e.variant,observations:status==='rejected'?[]:e.observations,novelty_candidates:e.novelty,review_note:e.note,source_fingerprint:i.source_fingerprint}};try{{const response=await fetch('/api/reviews/'+encodeURIComponent(i.report_id),{{method:'POST',headers:{{'Content-Type':'application/json','X-Review-Token':CSRF}},body:JSON.stringify(payload)}});const result=await response.json();if(!response.ok)throw new Error(result.error||'Could not save review');i.decision=result.decision;i._edit=null;s.className='status ok';s.textContent='Saved';metrics();renderFilters();setTimeout(render,250)}}catch(error){{s.className='status error';s.textContent=error.message}}}}
