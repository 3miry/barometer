...
from .temporal import temporal_priority
    ...
    try:
        ...
        columns = {
            row[1] for row in connection.execute(
                "PRAGMA table_info(complaints)").fetchall()
        }
        author_id = "author_id" if "author_id" in columns else "NULL AS author_id"
        author_handle = (
            "author_handle" if "author_handle" in columns
            else "NULL AS author_handle"
        )
        rows = connection.execute(
            "SELECT id,ts,source,model,variant,text,url,"
            f"{author_id},{author_handle} "
...
def build_review_items(
    ...
    *,
    ...
    for row in load_source_reports(source_db):
        ...
        priority = temporal_priority(row["text"])
        ...
        for index, (seed_family, seed_variant) in enumerate(targets, start=1):
            ...
            items.append({
                ...
                "temporal_priority": priority.as_dict(),
                "proposal": {
...
.filters{{display:flex;gap:7px;flex-wrap:wrap;align-items:center;margin-bottom:14px}}.filters button{{border:1px solid var(--line);background:var(--panel);color:var(--muted);padding:7px 10px;border-radius:999px}}.filters button.active{{background:#173441;color:var(--ink);border-color:#3f7485}}.filters select{{width:auto;margin-left:auto;padding:7px 30px 7px 10px;border-radius:999px}}
...
.source-control{{display:grid;grid-template-columns:minmax(180px,1fr) minmax(180px,1fr) auto;gap:10px;align-items:end}}.source-control .source-name{{font-weight:800;color:var(--blue);padding:9px 0}}.source-control button{{border:1px solid #74483f;background:#3a201b;color:var(--ink);border-radius:9px;padding:9px 12px;font-weight:750;white-space:nowrap}}.source-control button.restore{{background:#173a2a;border-color:#38694f}}
...
@media(max-width:850px){{.metrics{{grid-template-columns:1fr 1fr}}.layout{{grid-template-columns:1fr}}.queue{{max-height:260px;min-height:0}}.detail{{min-height:500px}}.obsgrid{{grid-template-columns:1fr 1fr}}.source-control{{grid-template-columns:1fr}}}}
...
<script>
const CSRF={token_json};let data={{items:[],meta:null}},filter='pending',sortMode='signal',selected=null;
...
function renderFilters(){{const names=['pending','reclassify','all','approved','corrected','rejected','deferred'];document.querySelector('#filters').innerHTML=names.map(n=>`<button class="${{filter===n?'active':''}}" data-filter="${{n}}">${{n}}</button>`).join('')+`<select id="queue-sort" aria-label="Queue order"><option value="signal" ${{sortMode==='signal'?'selected':''}}>Signal + temporal priority</option><option value="temporal" ${{sortMode==='temporal'?'selected':''}}>Temporal only</option><option value="newest" ${{sortMode==='newest'?'selected':''}}>Newest first</option></select>`;document.querySelectorAll('[data-filter]').forEach(b=>b.onclick=()=>{{filter=b.dataset.filter;selected=null;render()}});document.querySelector('#queue-sort').onchange=e=>{{sortMode=e.target.value;selected=null;render()}}}}
const eligibilityWeight=i=>({{behaviour_report:40,novel_candidate:30,ambiguous:20,uncodable_appraisal:15,excluded_adversarial:10,chatter:0}}[i.proposal.eligibility]??0);
function visible(){{const items=data.items.filter(i=>filter==='all'||decisionStatus(i)===filter);return items.sort((a,b)=>{{if(sortMode==='newest')return b.received_at.localeCompare(a.received_at);const scoreA=(sortMode==='signal'?eligibilityWeight(a):0)+a.temporal_priority.score,scoreB=(sortMode==='signal'?eligibilityWeight(b):0)+b.temporal_priority.score;return(scoreB-scoreA)||b.received_at.localeCompare(a.received_at)}})}}
...
function renderQueue(){{const q=document.querySelector('#queue'),scroll=q.scrollTop,items=visible();if(!items.some(i=>i.review_unit_id===selected))selected=items[0]?.review_unit_id||null;q.innerHTML=items.length?grouped(items).map(group=>{{const first=group[0],siblings=data.items.filter(i=>i.report_id===first.report_id),reviewed=siblings.filter(i=>!['pending','reclassify'].includes(decisionStatus(i))).length,split=first.target_count>1,p=first.temporal_priority;return`<div class="qgroup ${{split?'split':''}}">${{split?`<div class="qgroup-head">Comparison source <span class="qgroup-progress">· ${{first.target_count}} targets · ${{reviewed}}/${{first.target_count}} reviewed</span></div><div class="qsource-text">${{esc(first.text)}}</div>`:''}}${{group.map(i=>`<button class="qitem ${{selected===i.review_unit_id?'active':''}}" data-id="${{esc(i.review_unit_id)}}"><div class="qtop"><span class="qtarget">${{esc(i.target_label)}}</span><span><span class="pill ${{p.score>=4?'good':''}}">time ${{p.score}}</span> <span class="pill ${{['pending','reclassify'].includes(decisionStatus(i))?'warn':'good'}}">${{esc(decisionStatus(i))}}</span></span></div><div class="qid">${{esc(i.report_id)}}${{i.target_count>1?` · target ${{i.target_index}}/${{i.target_count}}`:''}}</div>${{split?'':`<div class="qtext">${{esc(i.text)}}</div>`}}</button>`).join('')}}</div>`}}).join(''):'<div class="empty" style="min-height:180px">Nothing in this view.</div>';q.scrollTop=scroll;document.querySelectorAll('.qitem').forEach(b=>b.onclick=()=>go(b.dataset.id))}}
...
function searchTermEditor(item,index){{const c=concept(item.concept_id);return`<div class="obs" data-search-term="${{index}}"><div class="obshead"><div class="obs-title"><b>${{esc(item.phrase)}}</b><span class="pill">${{esc(c?.label||item.concept_id)}}</span></div><button class="remove" data-remove-term="${{index}}">Remove</button></div></div>`}}
...
function renderDetail(){{
 ...
 i._edit=i._edit||{{family:saved?.target_family??i.seed_family,variant:saved?.target_variant??i.seed_variant,observations:clone(saved?.observations??i.proposal.observations),searchTerms:clone(saved?.search_term_candidates??[]),novelty:clone(saved?.novelty_candidates??i.proposal.novelty_candidates),note:saved?.review_note||''}};
 ...
 const suppression=i.source_suppression,reason=suppression?.reason||'marketing';
 const sourceControl=i.author_id?`<div class="section"><h2>Future source sampling</h2><div class="source-control"><div><label>X account</label><div class="source-name">${{i.author_handle?'@'+esc(i.author_handle):'author '+esc(i.author_id)}}</div></div><div><label>Reason</label><select id="suppression-reason">${{[['marketing','Marketing'],['repeated_chatter','Repeated chatter'],['automated','Automated posting'],['off_topic','Consistently off-topic'],['other','Other']].map(([value,label])=>`<option value="${{value}}" ${{value===reason?'selected':''}}>${{label}}</option>`).join('')}}</select></div><button id="toggle-suppression" class="${{suppression?.active?'restore':''}}">${{suppression?.active?'Restore future sampling':'Suppress future posts'}}</button></div><div class="sub">This is reversible and affects future collection only. It does not delete this report or change its coding.</div></div>`:'';
 ...
 d.innerHTML=`<div class="detail-nav"><div class="source-anchor">${{split?`Shared source<b>Target ${{i.target_index}} of ${{i.target_count}}</b>`:'Source report'}}<div class="shortcut-hint">J/K queue · [ / ] shared targets</div></div><div class="nav-buttons">${{targetNav}}${{goButton(queuePrev,'↑ Previous item')}}${{goButton(queueNext,'Next item ↓')}}</div></div><div class="eyebrow">${{esc(i.proposal.classifier_version)}} · ${{esc(i.received_at)}}</div><h1>${{esc(i.target_label)}} · ${{esc(i.source)}} report</h1><div class="badges"><span class="pill">stored: ${{esc(i.stored_variant||i.stored_family)}}</span><span class="pill ${{attrBad?'warn':'good'}}">${{esc(i.attribution_status)}}</span><span class="pill">${{esc(i.proposal.eligibility)}}</span><span class="pill ${{i.temporal_priority.score>=4?'good':''}}">temporal ${{i.temporal_priority.score}} · ${{esc(i.temporal_priority.band)}}</span>${{i.temporal_priority.cues.map(c=>`<span class="pill">${{esc(c)}}</span>`).join('')}}</div>${{split?`<div class="splitnote">This source mentions ${{i.target_count}} model targets. You are coding only <b>${{esc(i.target_label)}}</b> in this slice. The source-level classifier found ${{i.proposal.source_observation_count}} governed observation(s), but none were copied across models because direction and valence may differ.</div>`:(i.proposal.abstention_reason?`<div class="splitnote">${{esc(i.proposal.abstention_reason)}}</div>`:'')}}${{reclassify?'<div class="splitnote"><b>Reclassification required.</b> A saved observation uses an older problem-oriented concept. Use its replacement button, then review state and change again.</div>':''}}<div class="report">${{esc(i.text)}}</div>${{sourceControl}}<div class="section"><h2>Attribution for this slice</h2><div class="grid"><div><label>Target family</label><select id="family">${{Object.entries(data.meta.families).map(([k,v])=>`<option value="${{k}}" ${{k===e.family?'selected':''}}>${{esc(v.lab)}} · ${{esc(v.label)}}</option>`).join('')}}</select></div><div><label>Exact model</label><select id="variant">${{variantOptions(e.family,e.variant)}}</select></div></div></div><div class="section"><h2>Governed observations for ${{esc(i.target_label)}}</h2><div id="observations">${{e.observations.map(obsEditor).join('')||'<div class="sub">No governed observations selected for this model yet.</div>'}}</div><div class="addrow"><select id="add-concept"><option value="">Add a governed concept…</option>${{data.meta.concepts.filter(c=>c.status!=='superseded'&&!e.observations.some(o=>o.concept_id===c.id)).map(c=>`<option value="${{c.id}}">${{esc(c.label)}} · ${{esc(c.parent||c.reporting_layer)}}</option>`).join('')}}</select><button id="add">Add</button></div></div><div class="section"><h2>Search dictionary candidates</h2><div class="sub">Record the exact reusable phrase from this report and link it to an approved governed observation. Candidates remain private and cannot activate paid searches until separately reviewed.</div><div id="search-terms">${{e.searchTerms.map(searchTermEditor).join('')||'<div class="sub">No reusable search phrase recorded.</div>'}}</div><div class="addrow"><input id="new-search-term" placeholder="Exact phrase, e.g. nerfed"><select id="new-search-concept"><option value="">Link to governed observation…</option>${{e.observations.map(o=>`<option value="${{o.concept_id}}">${{esc(concept(o.concept_id)?.label||o.concept_id)}}</option>`).join('')}}</select><button id="add-search-term">Add phrase</button></div></div><div class="section"><h2>Unresolved novelty for this target</h2><label>Comma-separated private candidate labels</label><input id="novelty" value="${{esc(e.novelty.join(', '))}}"><label style="margin-top:10px">Reviewer note</label><textarea id="note">${{esc(e.note)}}</textarea></div><div class="actions">${{split?'':`<button class="approve" data-save="approved">Approve proposal</button>`}}<button class="correct" data-save="corrected">Save coding</button><button data-save="deferred">Defer</button><button class="reject" data-save="rejected">No attributable report</button><span class="status" id="save-status"></span></div>`;
 ...
 document.querySelectorAll('[data-remove]').forEach(b=>b.onclick=()=>{{const removed=e.observations.splice(Number(b.dataset.remove),1)[0];e.searchTerms=e.searchTerms.filter(t=>t.concept_id!==removed.concept_id);renderDetail()}});
 ...
 document.querySelectorAll('[data-remove-term]').forEach(b=>b.onclick=()=>{{e.searchTerms.splice(Number(b.dataset.removeTerm),1);renderDetail()}});
 document.querySelector('#add-search-term').onclick=()=>{{const phrase=document.querySelector('#new-search-term').value.trim(),conceptId=document.querySelector('#new-search-concept').value;if(!phrase||!conceptId){{showToast('Add a phrase and choose its governed observation.');return}}if(e.searchTerms.some(t=>t.phrase.toLowerCase()===phrase.toLowerCase()&&t.concept_id===conceptId)){{showToast('That phrase is already linked here.');return}}e.searchTerms.push({{phrase,concept_id:conceptId}});renderDetail()}};
 document.querySelectorAll('[data-save]').forEach(b=>b.onclick=()=>save(i,b.dataset.save));
 const suppressionButton=document.querySelector('#toggle-suppression');if(suppressionButton)suppressionButton.onclick=()=>setSuppression(i,!i.source_suppression?.active);
}}
let toastTimer;
...
async function save(i,status){{
 ...
 const payload={{status,target_family:e.family,target_variant:e.variant,observations:status==='rejected'?[]:e.observations,search_term_candidates:status==='rejected'?[]:e.searchTerms,novelty_candidates:status==='rejected'?[]:e.novelty,review_note:e.note,source_fingerprint:i.source_fingerprint}};
...
async function setSuppression(i,active){{
 const reason=document.querySelector('#suppression-reason').value,button=document.querySelector('#toggle-suppression');button.disabled=true;button.textContent='Saving…';
 try{{const response=await fetch('/api/source-suppressions',{{method:'POST',headers:{{'Content-Type':'application/json','X-Review-Token':CSRF}},body:JSON.stringify({{report_id:i.report_id,source_fingerprint:i.source_fingerprint,active,reason}})}});const result=await response.json();if(!response.ok)throw new Error(result.error||'Could not update source suppression');data.items.filter(item=>item.report_id===i.report_id).forEach(item=>item.source_suppression=result.suppression);render();showToast(active?'Future posts from this source are suppressed.':'Source restored for future sampling.')}}catch(error){{button.disabled=false;button.textContent=active?'Suppress future posts':'Restore future sampling';showToast(error.message)}}
}}
